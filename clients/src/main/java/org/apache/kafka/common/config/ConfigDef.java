...
public class ConfigDef {
    ...
    /**
     * 保存producer 或者 consumer 初始化时的配置信息
     */
    ...
    public ConfigDef define(ConfigKey key) {
    	log.info("【配置键值对设置:key.name="+ key.name + ", content="+ key.toString() +"】");
     ...
     * @param name          the name of the config parameter :配置参数的name
     * @param type          the type of the config  ： 配置类型
    ...
    /**
     * Define a new configuration with no default value and no special validation logic
     * @param name          The name of the config parameter  :配置参数名称字符串
     * @param type          The type of the config ：配置参数数据类型
     * @param importance    The importance of this config: is this something you will likely need to change.  ：配置参数的重要性
     * @param documentation The documentation string for the config  ：配置参数描述
     * @return This ConfigDef so you can chain calls
     * 定义一个新的配置在没有默认值并且没有指定确定的逻辑时 
     */
     ...
     */
    /**
     * 根据这个配置定义解析和验证用户提供的原始配置键值对信息，可确定的是原始键值对key是Stsring类型，但是value可以是字符串或者其他类型，解析可针对实现map的所有类，包括Properties
     * @param props  待校验和解析的键值对配置
     * @return
     */
    ...
    Object parseValue(ConfigKey key, Object value, boolean isSet) {
        ...
        // props map doesn't contain setting, the key is required because no default value specified - its an error
        	...
        	/**
        	 * 如果当前属性没有被用户设置，并且此值得默认值被设置成Object，则报错。
        	 */
        ...
        /**
         * 如果此值需要校验 ，则需要完成校验， 在ProducerConfig中的静态方法中ConfigDef调用define方法时 
         * 如果设置了Validator的实现类Range变量，则需要对此键值对做校验
         */
     ...
     * The config types
     * 
     * 用户在初始化map config时 map中value字符串的类型；例如BOOTSTRAP_SERVERS_CONFIG类型为多个ip地址以分号隔开，那么这个value对应的type为List
     */
    public enum Type {
     ...
     * 用户在初始化map config时 map时 当前配置的重要程度。
     */
     ...
     * 
     * 校验逻辑，若在初始化ConfigDef时 ，设置了此对象，则在解析原始配置键值对时需要对此configKey进行校验
     */
    ...
    /**
     * 用户在组建producer和consumer初始化设置map 得key value 封装，每个键值对对应一个ConfigKey对象
     * @author 001
     *
     */
    public static class ConfigKey {
        public final String name;//
        public final Type type;//配置值类型
        public final String documentation;//配置描述
        ...
        public ConfigKey(String name, Type type, Object defaultValue, Validator validator,
            ...
            this.name = name;
            this.type = type;
            //如果此默认值为Object则，将defaultValue设置为Object ，否则设置默认值
		...
		@Override
		public String toString() {
			return "ConfigKey [name=" + name + ", type=" + type + ", defaultValue="
					+ defaultValue + ", validator=" + validator + ", importance=" + importance + ", group=" + group
					+ ", orderInGroup=" + orderInGroup + ", width=" + width + ", displayName=" + displayName
					+ ", dependents=" + dependents + ", recommender=" + recommender + ", internalConfig="
					+ internalConfig + "]";
		}
