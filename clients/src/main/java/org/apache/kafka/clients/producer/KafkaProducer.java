...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    /**
     * 当用户没有设置client.id值时 ，在初始化生产者时 初始化一个，此时需要用到此变量
     */
     ...
     */
    /**
     *  生产者的有效可用配置项可以 参考http://kafka.apache.org/documentation.html#producerconfigs
     *  生产者初始化时需要的配置是通过键值对设置的
     * @param properties 生产者配置
     * @param keySerializer 消息key的序列化类，必须实现Serializer接口，当序列化类直接传入时 ，实现Serializer接口的configure不会再初始化生产者时被调用，参考
     *                      private KafkaProducer(ProducerConfig config, Serializer<K> keySerializer, Serializer<V> valueSerializer)中的代码
    ...
    private KafkaProducer(ProducerConfig config, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
        try {
            ...
            //存储当前生产者对应的解析后的配置对象ProducerConfig
            ...
            /**
             * 如果用户没有设置client.id值，则初始化一个
             */
            if (clientId.length() <= 0)
                clientId = "producer-" + PRODUCER_CLIENT_ID_SEQUENCE.getAndIncrement();
            /**
             * 用来生成只读 的衡量指标键值对map集合
             */
            ...
            MetricConfig metricConfig = new MetricConfig().samples(config.getInt(ProducerConfig.METRICS_NUM_SAMPLES_CONFIG))
                    ...
                    .tags(metricTags);
            log.info("【初始化衡量指标配置:"+ metricConfig.toString() +"】");
            ...
            log.info("【设置衡量指标："+ metrics.toString() +"】");
            
            /**
             * 设置发送消息的指定分区策略,如果不设置，默认用自带的DefaultPartitioner，参见 ProducerConfig中的static 方法初始化ConfigKey值代码
             */
            this.partitioner = config.getConfiguredInstance(ProducerConfig.PARTITIONER_CLASS_CONFIG, Partitioner.class);
            log.info("【设置发送消息指定分区策略类:"+  this.partitioner.getClass() +"】");
            /**
             * retry.backoff.ms
            ...
            if (keySerializer == null) {
                this.keySerializer = ensureExtended(config.getConfiguredInstance(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG,
                                                                                         Serializer.class));
                log.info("【若用户未设置keySerializer，则默认keySerializer为："+  this.keySerializer .getClass().getName() +"】");
            ...
            if (valueSerializer == null) {
                this.valueSerializer = ensureExtended(config.getConfiguredInstance(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG,
                                                                                           Serializer.class));
                log.info("【若用户未设置valueSerializer，则默认valueSerializer为："+  this.keySerializer .getClass().getName() +"】");
            ...
            log.info("【配置元数据为metadata:"+ metadata.toString() +"】");
            /**
            ...
            /**
             * 生产者可以用来缓冲等待被发送到服务器的记录的总字节数
             * producer可以用来缓存数据的内存大小。如果数据产生速度大于向broker发送的速度，producer会阻塞或者抛出异常，
             * 以“block.on.buffer.full”来表明。这项设置将和producer能够使用的总内存相关，但并不是一个硬性的限制，因为不是producer使用的所有内存都是用于缓存。
             * 一些额外的内存会用于压缩（如果引入压缩机制），同样还有一些用于维护请求。
             */
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);
            /**
             * 指定topic的压缩类型。除了支持’gzip’, ‘snappy’,‘lz4’外，还支持”uncompressed(不压缩)”以及producer(由producer来指定)
             ...
             * 以下设置为常用选项：（1）acks=0： 设置为0表示producer不需要等待任何确认收到的信息。副本将立即加到socket buffer并认为已经发送。
             * 没有任何保障可以保证此种情况下server已经成功接收数据，同时重试配置不会发生作用（因为客户端不知道是否失败）回馈的offset会总是设置为-1；
             * （2）acks=1： 这意味着至少要等待leader已经成功将数据写入本地log，但是并没有等待所有follower是否成功写入。这种情况下，如果follower没有成功备份数据，
             * 而此时leader又挂掉，则消息会丢失。（3）acks=all： 这意味着leader需要等待所有备份都成功写入日志，这种策略会保证只要有一个备份存活就不会丢失数据。这是最强的保证。，	string
             */
             ...
             *producer将会等待给定的延迟时间以允许其他消息记录发送，这些消息记录可以批量处理。这可以认为是TCP种Nagle的算法类似。这项设置设定了批量处理的更高的延迟边界：
             *一旦我们获得某个partition的batch.size，他将会立即发送而不顾这项设置，然而如果我们获得消息字节数比这项设置要小的多，我们需要“linger”特定的时间以获取更多的消息。
            ...
            this.accumulator = new RecordAccumulator(config.getInt(ProducerConfig.BATCH_SIZE_CONFIG),
                    ...
                    apiVersions,
                    transactionManager);
            log.info("【设置消息累加器对象RecordAccumulator：" + accumulator.toString() +"】");
            
            /**
             * 根据bootstrap.servers指定的多个ip地址创建addresses集合
             */
            ...
            log.info("【Sensor throttleTimeSensor:"+ throttleTimeSensor.toString() +"】");
            /**
             * connections.max.idle.ms关闭连接空闲时间
             * reconnect.backoff.ms 连接失败时，当我们重新连接时的等待时间。这避免了客户端反复重连
             * reconnect.backoff.max.ms 
             * send.buffer.bytes发送数据时的缓存空间大小
            ...
            NetworkClient client = new NetworkClient(
                    ...
                    apiVersions,
                    throttleTimeSensor);
            log.info("【NetowrkClient client:"+ client.toString() + "】");
            /**
             * 设置发送线程
             */
     ...
     *
     * @param record The record to send : record对象
     * @param callback A user-supplied callback to execute when the record has been acknowledged by the server (null
     *        indicates no callback) :回调方法，消息发送成功后来调用。注意此回调设计模式的使用
     *
    ...
    public Future<RecordMetadata> send(ProducerRecord<K, V> record, Callback callback) {
    	...
    	/**
    	 * interceptors 拦截record，如果在初始化 KafkaProducer时在Properties中设置了拦截器集合(interceptor.classes)，
    	 * 则发送记录到kafka之前可以在拦截器的onSend方法中做一些处理操作，使用方法可参考KafkaProducerTest的testInterceptorConstructClose方法
    	 */
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
        	log.info("【KakkaProducer doSend maxBlockTimeMs："+ maxBlockTimeMs +"】");
            // first make sure the metadata for the topic is available
            ClusterAndWaitTime clusterAndWaitTime = waitOnMetadata(record.topic(), record.partition(), maxBlockTimeMs);
            log.info("【KakkaProducer doSend clusterAndWaitTime："+ clusterAndWaitTime +"】");
            long remainingWaitMs = Math.max(0, maxBlockTimeMs - clusterAndWaitTime.waitedOnMetadataMs);
            log.info("【KakkaProducer doSend remainingWaitMs："+ remainingWaitMs +"】");
            ...
            log.info("【KakkaProducer doSend tp："+ tp.toString() +"】");
            ...
            log.info("【KakkaProducer doSend serializedSize："+ serializedSize +"】");
            ...
            log.info("【KakkaProducer doSend result："+ result.toString() +"】");
            /**
             * 如果已经达到设置的上限 ，则发送数据
             */
            if (result.batchIsFull || result.newBatchCreated) {
            	log.info("【累加器缓存数据已满，开始唤醒nioSelector】");
