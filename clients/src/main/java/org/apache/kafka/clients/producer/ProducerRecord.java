...
public class ProducerRecord<K, V> {
    ...
    private final String topic;//消息将发送到哪个主题上
    private final Integer partition;//设置消息发送到当前主题的哪个分区上，此值不能为负数
    private final Headers headers;//包含在消息中的头 信息
    private final K key;//消息包含的key
    private final V value;//消息内容
    private final Long timestamp;//当前消息的时间戳

    /**
     * Creates a record with a specified timestamp to be sent to a specified topic and partition
     * 
     * @param topic The topic the record will be appended to ：消息将发送到哪个主题上
     * @param partition The partition to which the record should be sent:设置消息发送到当前主题的哪个分区上，此值不能为负数
     * @param timestamp The timestamp of the record ：当前消息的时间戳
     * @param key The key that will be included in the record  ：消息包含的key
     * @param value The record contents：消息内容
     * @param headers the headers that will be included in the record:包含在消息中的头 信息
     */
