...
public class ProducerConfig extends AbstractConfig {
     ...
     */
    static {
        CONFIG = new ConfigDef().define(BOOTSTRAP_SERVERS_CONFIG, Type.LIST, Importance.HIGH, CommonClientConfigs.BOOTSTRAP_SERVERS_DOC)//bootstrap.servers
                                .define(BUFFER_MEMORY_CONFIG, Type.LONG, 32 * 1024 * 1024L, atLeast(0L), Importance.HIGH, BUFFER_MEMORY_DOC)//buffer.memory默认为32 * 1024 * 1024L，最小值为0
                                .define(RETRIES_CONFIG, Type.INT, 0, between(0, Integer.MAX_VALUE), Importance.HIGH, RETRIES_DOC)//retries 重发次数，值必须在0到最大int值
                                .define(ACKS_CONFIG,
                                        Type.STRING,
                                        "1",
                                        in("all", "-1", "0", "1"),
        ...
        log.info("【初始化设置producerconfig配置为:"+ CONFIG.configKeys().toString() +"】");
    }
    ...
    /**
     *  此方法作用往初始化配置属性对象中添加key和value的序列化实现类名称
     * @param properties
     * @param keySerializer
    ...
    /**
     * 根据用户设置的配置属性map初始化ProducerConfig
     * @param props
     */
